...
2. **Post-approval revalidation.** After a grant is consumed, the loop re-observes, uniquely re-grounds the **approved semantic identity** in the fresh scene (opaque `eN` is reminted each `observePage` and is not a capability), re-validates that live target, re-derives the binding, and executes only if security identity still matches. Unique equivalent replacement may proceed. Semantic swap, action/risk/origin/frame change, missing target, or two plausible replacements → block. Failure does not retry with more context.
